...
const NotifyBar=({user,orgName})=>{
    const router=useRouter()
    return(
        <div onClick={()=>{router.push(`/chatpen/${user}?cs=1&uid=${encodeURIComponent(orgName)}`)}} className="flex cursor-pointer items-center px-4 py-2 hover:bg-[#fce3e3]">
